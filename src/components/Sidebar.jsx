...
export default function Sidebar({ activePage, onNavigate, collapsed, onToggle }) {
  ...
  return (
    <aside className={`bg-[#111C4A] text-white flex flex-col transition-all duration-300 ${collapsed ? "w-16" : "w-64"}`}>
      {/* Header */}
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium transition-all ${
              activePage === item.path ? "bg-white/15 text-white" : "text-blue-200/70 hover:bg-white/5 hover:text-white"
            }`}
